...
def train(model, device, data_loader, optimizer, opt):
    ...
    for epoch in range(opt['last_epoch'], EPOCHS+1):  # loop over the dataset multiple times
        t_loss, tp, tn, fp, fn = 0., 0, 0, 0, 0
        for batch_train in data_loader:
            # get the inputs;
            ...
            if opt['model']['problem'] == 'regression':
                labels = torch.round(labels, decimals=0)
...
if __name__ == "__main__":
    ...
    if opt['train_from_last_epoch'] and os.path.exists(opt['model_checkpoints']):
        try:
            ...
            last_ep = int(files[0].lstrip('epoch-').rstrip('.pt'))
            opt['last_epoch'] = last_ep + 1
